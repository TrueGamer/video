...
public class SmallAdViewHolder extends BaseMuiltyViewHolder<TTFeedSmallPicAd> {
    ...
    @Override
    public void init(TTFeedSmallPicAd ad, BaseViewHolder helper, Context context) {

        RelativeLayout rlADSmallPic = (RelativeLayout) helper.getView(R.id.rlADSmallPic);
        ...
        ImageView mSmallImage = (ImageView) helper.getView(R.id.iv_listitem_image);
        ...
        if (ad.getImageList() != null && !ad.getImageList().isEmpty()) {
            ...
            if (image != null && image.isValid()) {
                mAQuery.id(mSmallImage).image(image.getImageUrl());
            }
        }

        //可以被点击的view, 也可以把convertView放进来意味item可被点击
        List<View> clickViewList = new ArrayList<View>();
        clickViewList.add(rlADSmallPic);
...
//            creativeViewList.add(convertView);
        ...
        ttFeedAd.registerViewForInteraction((ViewGroup) rlADSmallPic, clickViewList, creativeViewList,
    ...
    private void bindDownloadListener(final Button adCreativeButton ,final Button mStopButton , Button mRemoveButton , TTFeedAd ad) {
        TTAppDownloadListener downloadListener = new TTAppDownloadListener() {
            ...
            private boolean isValid() {
                return mTTAppDownloadListenerMap.get("SmallAdViewHolder") == this;
            }
        };
        //一个ViewHolder对应一个downloadListener, isValid判断当前ViewHolder绑定的listener是不是自己
        ad.setDownloadListener(downloadListener); // 注册下载监听器
        mTTAppDownloadListenerMap.put("SmallAdViewHolder", downloadListener);
    }
}
